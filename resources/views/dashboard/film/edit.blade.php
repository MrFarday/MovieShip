     ...
     <div class="container mx-auto">
         <div class="bg-white border border-4 rounded-lg shadow relative m-10">
          ...
          <div class="p-6 space-y-6">
              <form action="{{ route('dashboard.film.update', $film) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <input type="text" value="{{$film->id}}" hidden>
                  <div class="grid grid-cols-6 gap-6">
                      <div class="col-span-full">
                          <label for="title" class="text-sm font-medium text-gray-900 block mb-2">Title</label>
                          <input type="text" name="title" id="title" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="Masukkan title....”" value="{{ $film->title }}">
                      </div>
                      <div class="col-span-full">
                          <label for="description" class="text-sm font-medium text-gray-900 block mb-2">Deskripsi</label>
                          <textarea id="product-details" rows="6" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-4" placeholder="Deskripsi ..." name="description">{{ $film->description }}</textarea>
                      </div>
                      <div class="col-span-2">
                         <label for="release_year" class="text-sm font-medium text-gray-900 block mb-2">Tahun Rilis</label>
                         <input type="year" name="release_year" id="release_year" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="Masukkan Tahun Liris...." value="{{ $film->release_year }}"  required>
                     </div>
                     <div class="col-span-2">
                         <label for="length" class="text-sm font-medium text-gray-900 block mb-2">Durasi Film</label>
                         <input type="text" name="length" id="length" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="Masukkan Durasi Film...." value="{{ $film->length }}" required>
                     </div>
                     <div class="col-span-2">
